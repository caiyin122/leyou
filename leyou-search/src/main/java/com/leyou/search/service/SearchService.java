...
public class SearchService {
   ...
   public Goods buildGoods(Spu spu) throws IOException {
       ...
       skus.forEach(sku -> {
           ...
           //获取sku的图片，数据库的图片可能是多张，多张以“，”分隔，以逗号切割返回图片数组，获取第一张图片
       ...
       params.forEach(param -> {
           ...
           //判断规格参数的类型
           if(param.getGeneric()){
                //如果是通用类型的参数就，就从genericSpecMap获取规格参数值
               String value = genericSpecMap.get(param.getId().toString()).toString();

               //判断是否是数值类型，如果是应该返回一个数值区间
               if(param.getNumeric()){
               ...
               //如果是特殊的参数，就从specialSpecMap中去获取值
